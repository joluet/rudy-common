...
exports.buildConnections = (routeStops, routeState) => {
    return routeStops
        ...
        if (index < array.length - 1) {
            ...
            if (prevStop && nextStop && origin.itemType === types_1.ItemType.FerryTerminalDeparture) {
                result.push({
            ...
            else if (prevStop && nextStop && origin.itemType === types_1.ItemType.FerryTerminalTransit) {
                result.push({
                    id: `${prevStop.id}*${nextStop.id}*${origin.id}*${destination.id}`,
                    origin: origin,
                    destination: destination,
